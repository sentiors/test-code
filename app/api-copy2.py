...
from .models import User, Lab, GradingResult, LabSession, Group, Class
...
def show_results():
    try:
        ...
        for result in results:
            if result.timestamp:
                # Langsung format saja tanpa tambah jam
                result.local_timestamp = result.timestamp
            else:
                result.local_timestamp = '-'

            user_data = db_session.query(User).filter(User.username == result.username).first()
            gName = (user_data.group_name or '').strip() if user_data else ""
            ...
            if only_number and 1 <= int(only_number) <= 9:
                    ...
                    # Jika bukan 1-9 atau bukan angka, tampilkan apa adanya atau kosongkan
                    result.display_group = gName if gName else "-"
        ...
        print("Error:", str(e))
        return jsonify({"error": "Gagal mengambil data hasil", "details": str(e)}), 500
